												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<c:if test="${pd.STATUS == 1 }">
														<a class="btn btn-mini btn-purple" onclick="exportExcel('${pd.DB_DATA_ID}','${pd.BATCH_NUM}');">
															<i class="icon-pencil"></i>导出Excel
														</a>
														<a class="btn btn-mini btn-purple" onclick="exportTXT('${pd.DB_DATA_ID}','${pd.BATCH_NUM}');">
															<i class="icon-pencil"></i>导出TXT
														</a>
													</c:if>
...
$(function(){ 
	var t1 = window.setInterval(updateStatus,5000); 
});
				...
				if(result != ''){
					...
					for(var j = 0;j < db_data.length;j++){
						...
						if(status == '0'){
							...
							statusDivContent = '<span style="color: green">标化中</span>';
						}
...
function exportExcel(DB_DATA_ID,BATCH_NUM){
	var DB_DATA_TYPE = '${pd.DB_DATA_TYPE}';
	window.location.href='<%=basePath%>mtsDBData/exportExcelForPRKS23.do?DB_DATA_ID='+DB_DATA_ID+'&BATCH_NUM='+BATCH_NUM+'&DB_DATA_TYPE='+DB_DATA_TYPE;
}

function exportTXT(DB_DATA_ID,BATCH_NUM){
	var DB_DATA_TYPE = '${pd.DB_DATA_TYPE}';
	window.location.href='<%=basePath%>mtsDBData/exportTXTForPRKS23.do?DB_DATA_ID='+DB_DATA_ID+'&BATCH_NUM='+BATCH_NUM+'&DB_DATA_TYPE='+DB_DATA_TYPE;
}

function standardizeData(DB_DATA_ID,msg){
	var DB_DATA_TYPE = '${pd.DB_DATA_TYPE}';
	/* $("#"+DB_DATA_ID+"_standDataDiv").style.display='none'; */
	
	statusDivContent = '<span style="color: red">标化中</span>';
	$("#"+DB_DATA_ID+"_status").html(statusDivContent);

	$.ajax({ 
		type: "post", 
		url: '<%=basePath%>mtsDBData/standardizeData.do?DB_DATA_ID='+DB_DATA_ID+'&DB_DATA_TYPE='+DB_DATA_TYPE, 
		cache:false, 
		async:true, 
