			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="mtsDBDataUnstructured/listMtsDBDataUnstructured.do" method="post" name="mtsDBDataForm" id="mtsDBDataForm">
...
<%-- 								<td>
								  ...
								  <select  name="FLAG" id="FLAG"  style="width:150px;" >
									...
									<c:forEach items="${listDataSource}" var="DATA_SOURCE">
										<c:if test="${pd.FLAG == null }">
											<option value="${DATA_SOURCE.FLAG }" >${DATA_SOURCE.DESCRIPTION }</option>
										</c:if>
										<c:if test="${pd.FLAG != null }">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover"  style="margin-top:5px;">
							...
							<!-- 开始循环 -->	
							<c:choose>
								<c:when test="${not empty listMtsDBDataUnstructured}">
									<c:forEach items="${listMtsDBDataUnstructured}" var="pd" varStatus="vs">
										<tr>
											<%-- <td class='center' style="width: 30px;">
												<label><input type='checkbox' name='ids' value="${pd.DATA_UNSTRUCTURED_ID}" class="ace"/><span class="lbl"></span></label>
											</td> --%>
											...
											<td class="center">
												<div class="hidden-sm hidden-xs btn-group">
													<a class="btn btn-mini btn-purple" onclick="standardizeData('${pd.DATA_UNSTRUCTURED_ID}','${pd.IMPORT_NAME}');">
														<i class="icon-pencil"></i>标准化
													</a>
													<a class="btn btn-mini btn-success" onclick="viewResult('${pd.DATA_UNSTRUCTURED_ID}','${pd.BATCH_NUM}','${pd.STATUS}');">
														<i class="icon-pencil"></i>详细
													</a>
													<c:if test="${pd.STATUS == 1 }">
														<a class="btn btn-mini btn-purple" onclick="exportExcel('${pd.DATA_UNSTRUCTURED_ID}','${pd.BATCH_NUM}','${pd.STATUS}');">
															<i class="icon-pencil"></i>导出
														</a>
													</c:if>
													<a class="btn btn-xs btn-danger" onclick="delMtsDBData('${pd.DATA_UNSTRUCTURED_ID}','${pd.IMPORT_NAME }');">
...
function exportExcel(DATA_UNSTRUCTURED_ID,BATCH_NUM,STATUS){
	window.location.href='<%=path%>/mtsDBDataUnstructured/exportExcelUnstructured.do?DATA_UNSTRUCTURED_ID='+DATA_UNSTRUCTURED_ID+'&BATCH_NUM='+BATCH_NUM+'&STATUS='+STATUS;
}
...
function viewResult(DATA_UNSTRUCTURED_ID,BATCH_NUM,STATUS){
	top.jzts();
	 var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.URL = '<%=path%>/mtsDBDataUnstructured/listMtsDBDataDetailUnstructured.do?DATA_UNSTRUCTURED_ID='+DATA_UNSTRUCTURED_ID+'&BATCH_NUM='+BATCH_NUM+'&STATUS='+STATUS;
	 diag.Title ="标化结果详细";
...
function downloadTemplate(){
	window.location.href='<%=basePath%>mtsDBDataUnstructured/downloadTemplate.do';
}
...
function importExcel(){
	 ...
	 diag.URL = '<%=path%>/mtsDBDataUnstructured/goUploadExcelUnstructured.do';
	 diag.Width = 1000;
	 diag.Height = 800;
	 diag.CancelEvent = function(){ //关闭事件
...
function standardizeData(DATA_UNSTRUCTURED_ID,msg){
	bootbox.confirm("确定要标准化["+msg+"]的数据信息吗?", function(result) {
		if(result) {
			top.jzts();
			var url = "<%=path%>/mtsDBDataUnstructured/standardizeDataUnstructured.do?DATA_UNSTRUCTURED_ID="+DATA_UNSTRUCTURED_ID;
			$.get(url,function(data){
				nextPage(${page.currentPage});
			});
		};
...
//删除
function delMtsDBData(DATA_UNSTRUCTURED_ID,msg){
	bootbox.confirm("确定要删除["+msg+"]的数据信息吗?", function(result) {
		if(result) {
			top.jzts();
			var url = "<%=path%>/mtsDBDataUnstructured/delMtsDBDataUnstructured.do?DATA_UNSTRUCTURED_ID="+DATA_UNSTRUCTURED_ID;
			$.get(url,function(data){
				nextPage(${page.currentPage});
			});
		};
