										...
										<tr>
											...
											<td class="center">
												<div class="hidden-sm hidden-xs btn-group">
													<a class="btn btn-mini btn-purple" onclick="manualStandardize('${pd.RECORD_DETAIL_ID}','${pd.BATCH_NUM}','${pd.NLP_RESULT}');">
...
function manualStandardize(infoId,batchNum,NLP_RESULT){
	top.jzts();
	var diag = new top.Dialog();
	diag.Drag=true;
	diag.URL = '<%=path%>/common/diagAll.do?ONTO_TYPE=1&id='+infoId +'&bathNum='+batchNum +'&NLP_RESULT='+NLP_RESULT;
	diag.Title ="手动标化页面";
...
function transferData(id,bathNum,res){
	...
	var XY_STANDARD_WORD = "";
	var XY_STANDARD_MAIN_CODE = "";
	var XY_STANDARD_ATTACH_CODE = "";
	var ZY_STANDARD_WORD = "";
	var ZY_STANDARD_MAIN_CODE = "";
	var SS_STANDARD_WORD = "";
	var SS_STANDARD_MAIN_CODE = "";
	
	
	for(var i in xqo){
		var onto_type = xqo[i].onto_type;
		if(onto_type == 1){
			XY_STANDARD_WORD = xqo[i].name;
			XY_STANDARD_MAIN_CODE = xqo[i].main_code;
			XY_STANDARD_ATTACH_CODE = xqo[i].add_code;
		}else if(onto_type == 2){
			SS_STANDARD_WORD = xqo[i].name;
			SS_STANDARD_MAIN_CODE = xqo[i].main_code;
		}else if(onto_type == 8){
			ZY_STANDARD_WORD = xqo[i].name;
			ZY_STANDARD_MAIN_CODE = xqo[i].main_code;
		}
	}
	
	var infoId = id;
	var batchNum = bathNum;
	
	/* alert("XY_STANDARD_WORD===>>"+XY_STANDARD_WORD);
	alert("XY_STANDARD_MAIN_CODE===>>"+XY_STANDARD_MAIN_CODE);
	alert("XY_STANDARD_ATTACH_CODE===>>"+XY_STANDARD_ATTACH_CODE);
	alert("ZY_STANDARD_WORD===>>"+ZY_STANDARD_WORD);
	alert("ZY_STANDARD_MAIN_CODE===>>"+ZY_STANDARD_MAIN_CODE);
	alert("SS_STANDARD_WORD===>>"+SS_STANDARD_WORD);
	alert("SS_STANDARD_MAIN_CODE===>>"+SS_STANDARD_MAIN_CODE); */
	$.ajax({
		url : '<%=path%>/mtsDBData/editMtsRecordInfo.do',  
		async: false,
        type : 'post',  
        data : {"infoId" : infoId ,"batchNum" : batchNum , "XY_STANDARD_WORD":XY_STANDARD_WORD ,  "XY_STANDARD_MAIN_CODE":XY_STANDARD_MAIN_CODE, "XY_STANDARD_ATTACH_CODE":XY_STANDARD_ATTACH_CODE , "ZY_STANDARD_WORD":ZY_STANDARD_WORD , "ZY_STANDARD_MAIN_CODE":ZY_STANDARD_MAIN_CODE , "SS_STANDARD_WORD":SS_STANDARD_WORD , "SS_STANDARD_MAIN_CODE":SS_STANDARD_MAIN_CODE },  
        dataType : 'json',  
