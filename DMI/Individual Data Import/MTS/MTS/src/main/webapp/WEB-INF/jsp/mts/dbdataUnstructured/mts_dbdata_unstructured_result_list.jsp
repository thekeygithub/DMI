			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
						<!-- 检索  -->
						<form action="mtsDBDataUnstructured/listMtsDBDataResultUnstructured.do" method="post" name="mtsDBDataForm" id="mtsDBDataForm">
						<input type="hidden" name="DATA_UNSTRUCTURED_ID" id="DATA_UNSTRUCTURED_ID" value="${DATA_UNSTRUCTURED_ID}"/>
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover"  style="margin-top:5px;">
							...
							<!-- 开始循环 -->	
							<c:choose>
								<c:when test="${not empty listUnstructuredRelevance}">
									<c:forEach items="${listUnstructuredRelevance}" var="pd" varStatus="vs">
										<tr>
											<td class='center' style="width: 30px;">${vs.index+1+ pcount}</td>
											<td class="left" style="width: 300px;">${pd.dname}</td>
											<td class="center">${pd.nlpStr} </td>
											<td class="center">${pd.mtsStr} </td>
						...
						<tr>
							<c:if test="${STATUS == 1 }">
								<td style="vertical-align:top;">
									<a class="btn btn-mini btn-success" onclick="exportExcel('${DATA_UNSTRUCTURED_ID}','${BATCH_NUM}');">导出结果</a>
								</td>
							</c:if>
...
function exportExcel(DATA_UNSTRUCTURED_ID,BATCH_NUM){
	window.location.href='<%=basePath%>mtsDBDataUnstructured/exportExcelUnstructured.do?DATA_UNSTRUCTURED_ID='+DATA_UNSTRUCTURED_ID+'&BATCH_NUM='+BATCH_NUM;
}
