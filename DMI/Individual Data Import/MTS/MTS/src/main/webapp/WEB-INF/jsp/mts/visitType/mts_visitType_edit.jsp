						...
						<div class="col-xs-12">
								<form action="mtsVisitType/${msg}.do" name="mtsVisitTypeAddForm" id="mtsVisitTypeAddForm" method="post">
									...
									<table id="table_report" class="table table-striped table-bordered table-hover">
										...
										<tr>
											<td style="width:100px;text-align: right;padding-top: 13px;">就诊类型名称:</td>
...
<script type="text/javascript">
	...
	function save(){		
		var msg = '${msg}';
		var OPERATE = '';
		var VISIT_TYPE_ID = '${pd.VISIT_TYPE_ID}';
		if(msg == 'saveVisitType'){
			OPERATE = 'Add';
		}else if(msg == 'editVisitType'){
			OPERATE = 'Edit';
		}
		...
		var result = 'true';
		$.ajax({
			url : '<%=path%>/mtsVisitType/verifyMtsVisitType.do',  
			async: false,
	        type : 'post',  
	        data : {"FLAG" : obj1.value ,"OPERATE" : OPERATE , "VISIT_TYPE_ID":VISIT_TYPE_ID},  
	        dataType : 'json',  
		...
		$.ajax({
			...
			async: false,
	        type : 'post',  
	        data : {"DESCRIPTION" : obj.value ,"OPERATE" : OPERATE, "VISIT_TYPE_ID":VISIT_TYPE_ID},  
	        dataType : 'json',  
	        success : function (data) {  
	        	if(data.result == 'false'){
	        	   $("#DESCRIPTION").tips({
