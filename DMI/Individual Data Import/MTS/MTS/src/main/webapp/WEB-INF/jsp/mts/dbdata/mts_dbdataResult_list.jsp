			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
						<!-- 检索  -->
						<form action="mtsDBData/listMtsDBDataResult.do" method="post" name="mtsDBDataForm" id="mtsDBDataForm">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover"  style="margin-top:5px;">
							<thead>
								<tr>
									...
									<th class="center">原始数据</th>
									<th class="center">NLP 结果</th>
									<th class="center">切词顺序</th>
								...
								<c:when test="${not empty listMtsDBdataDetail}">
									<c:forEach items="${listMtsDBdataDetail}" var="pd" varStatus="vs">
										<tr>
											...
											<td class="center">${pd.DIAG_NAME}</td>
											<td class="center">${pd.NLP_RESULT}</td>
											<th class="center">${pd.NLP_ORDER}</th>
											<td class="center">${pd.RESULT_1} 【${pd.RESULT_2}】【${pd.RESULT_3}】</td>
											<td class="center">
												<c:if test="${pd.STANDARD_RESULT == 0 }">
													<span style="color: red">未匹配</span>
												</c:if>
												<c:if test="${pd.STANDARD_RESULT == 1 }">
													<span style="color: green">完全匹配</span>
												</c:if>
												<c:if test="${pd.STANDARD_RESULT == 2 }">
													<span style="color: red">无法干预</span>
												</c:if>
											</td>
