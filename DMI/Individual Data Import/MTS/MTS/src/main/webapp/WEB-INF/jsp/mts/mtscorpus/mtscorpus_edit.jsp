		...
		<div class="main-content">
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
							<form action="mtsCorpus/editMtsCorpus.do" id="mtsCorpusEditForm"
								method="post">
								<input type='hidden' id="origContent_" name="origContent" value="" /> 
								<input type='hidden' id="jsonContent_" name="jsonContent" /> 
								<input type='hidden' id="corpusId_" name="corpusId" value="${mtsCorpus.ID}" />
								<div id="zhongxin" style="padding-top: 13px;">
									<div id="contentDiv" contentEditable="true" style="height:500px;overflow-y:scroll;border: 1px solid;">
									</div>
									<div>
...
function save() {
	...
	$("#tab tr").each(
			function(i) {
				...
				entity['spanNum'] = pnum;
				var corpusKey = pnum.split("_")[0]+"_"+origCorpus;
				if(corpusMap.get(corpusKey)){//如果map中能找到原始语料，在当前原始语料中添加元素
					entityArray = corpusMap.get(corpusKey);
					entityArray.push(entity);
				}else{
					entityArray =  new Array();
					entityArray.push(entity);
					corpusMap.put(corpusKey,entityArray);
				}
			});
	
	var listDetail = new Array();
	var corpusArray = corpusMap["elements"];
	//alert(JSON.stringify(corpusArray)+"===="+corpusArray.length);
	for(var j=0;j<corpusArray.length;j++){
		var obj = {};
		obj["origCorpus"] = corpusArray[j]["key"];
		obj["entityList"] = corpusArray[j]["value"];
		obj["pid"] = obj["origCorpus"].split("_")[0];
		var pid = obj["origCorpus"].split("_")[0];
		obj["phtml"] = $("#"+pid).prop("outerHTML");
		//alert(pid+"========"+obj["phtml"]+"====="+obj["entityList"]);
		listDetail.push(obj);
	}
	jsondata['listDetail'] = listDetail;
	
	jsondata['fileFullName'] = '${fileFullName}';
	jsondata['fileName'] = '${fileName}';
	jsondata['new_content'] = $("#contentDiv").html();
	jsondata['orig_content'] = '${html}';
	//alert(corpusMap.size());
	$("#jsonContent_").val(JSON.stringify(jsondata));
	if (flag == true) {
		$("#mtsCorpusEditForm").submit();
		$("#zhongxin").hide();
		$("#zhongxin2").show();
	}
}
