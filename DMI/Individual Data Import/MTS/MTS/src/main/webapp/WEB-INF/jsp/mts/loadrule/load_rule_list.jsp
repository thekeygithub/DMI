			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="loadRule/listLoadRules.do" method="post" name="loadRuleForm" id="loadRuleForm">
						<table style="margin-top:5px;">
							<tr>								
								
								<td style="padding-left:2px;">
								  <select  name="DATA_CLASS_CODE" id="DATA_CLASS_CODE"  style="width:120px;" >
								...
								<td style="padding-left:2px;">								
								  ...
								  <select  name="AREA_ID" id="AREA_ID"  style="width:120px;" >
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover"  style="margin-top:5px;">
							<thead>
								<tr>
									...
									<th class="center">标准化类型</th>
									<th class="center">key生成规则</th>
									<th class="center">value生成规则</th>
							...
							<!-- 开始循环 -->	
							<c:choose>
								<c:when test="${not empty listLoadRules}">
									<c:forEach items="${listLoadRules}" var="rule" varStatus="vs">
										<tr>
											<%-- <td class='center' style="width: 30px;">
												<label><input type='checkbox' name='ids' value="${rule.LOAD_RULE_ID}" class="ace"/><span class="lbl"></span></label>
											</td> --%>
											<td class='center' style="width: 30px;">${vs.index+1}</td>
											<td class="center">${rule.LOAD_RULE_NAME}</td>
											<td class="center">${rule.AREA_NAME}</td>
											<td class="center">${rule.DATA_CLASS_NAME}</td>
											<td class="center">${rule.DATA_TYPE_NAME}</td>
											<td class="center">${rule.KEY_GEN_RULE}</td>
											<td class="center">${rule.VALUE_GEN_RULE }</td>
											<td class="center">
												<div class="hidden-sm hidden-xs btn-group">
													<a class="btn btn-xs btn-success" title="编辑" onclick="editLoadRule('${rule.LOAD_RULE_ID}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
													</a>
													<a class="btn btn-xs btn-danger" onclick="delLRule('${rule.LOAD_RULE_ID}');">
...
function add(){
	 ...
	 diag.Drag=true;
	 diag.Title ="新增";
	 diag.URL = '<%=path%>/loadRule/goAddRule.do';
	 diag.Width = 520;
	 diag.Height = 510;
...
//修改
function editLoadRule(rule_id){
	 top.jzts();
	 var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="修改规则";
	 diag.URL = '<%=path%>/loadRule/goEditLR.do?rule_id='+rule_id;
	 diag.Width = 520;
	 diag.Height = 510;
	 diag.CancelEvent = function(){ //关闭事件
		if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
