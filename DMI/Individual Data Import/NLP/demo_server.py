...
define("port", default=10011, help="run on the given port", type=int)
...
def entities2JsonForm(entities_str):
    result = []
    #entities_str = entities_str.decode('utf8')#decode
    entities = entities_str.split('<br/>')#split
    #ent_re = re.compile(u'(.+)\u3010(.+)\u3011')#match
    ent_re = re.compile(u'(.+)\xe3\x80\x90(.+)\xe3\x80\x91')#match
    for entity_i in entities:
        match = ent_re.search(entity_i.strip())
        if match:
            content, enttype = match.group(1), match.group(2)
            result.append((content, enttype))
        else:
            continue

    data = {}
    for item in result:
        if not data.has_key(item[1]):
            data[item[1]] = [item[0]]
        elif item[0] not in data[item[1]]:
...
class IndexHandler(tornado.web.RequestHandler):
    # def get(self):
    #     source = self.get_argument('s', 'None')
    #     source_text = urllib.unquote(source).encode('utf-8')
    #     taggerB, taggerC = setModel('0')
    #     ner_result, ner_term_result = mainfunction(source_text, taggerB, taggerC)
    #     result = entities2JsonForm(ner_term_result)
    #     self.write(result)
    #     tools.addNewTerms(ner_term_result, ebao_dic, newtermfile, cn_en_dic)

    def post(self):
        source = self.get_argument("s", '')
        try:
            source_text = source.encode('utf8')
        except Exception, e:
            source_text = source
        print len(source_text)
        taggerB, taggerC = setModel('0')
        ner_result, ner_term_result, segment = mainfunction(source_text, taggerB, taggerC)
        #result = {'entity': entities2JsonForm(ner_term_result), 'segment': segment}
        result = {'entity': ner_term_result, 'segment': segment}
        self.write(tornado.escape.json_decode(json.dumps(result)))
        tools.addNewTerms(ner_term_result, ebao_dic, newtermfile, cn_en_dic)
