...
<head>
    ...
    <script type="text/javascript">
		...
		function addMenu() {
			...
			var menuData = {"modal.id":mid,"modal.title":title,"modal.pid":pid,"modal.ordernum":ordernum,"modal.url":url,"modal.image":image,"modal.whetherpublic":whetherpublic,"modal.actiontype":actiontype,"modal.remark":remark};
			return menuData;
		}  
		function selectIcon(){
		 	if (winicons) {
        ...
        $(function (){
        	form = $("#form2").ligerForm({
                ...
                fields: [ 
                    ...
                    { label: "打开方式", name: "actiontype", newline: true, type: "select", validate: { required: true }, dictionary: '刷新当前页面|tab页打开|弹出窗口',
                    	editor: {
							width : 180, 
				            selectBoxWidth: 200,
				            selectBoxHeight: 200,
   ...
   function  getMenuMess(menuid){//获取单个菜单信息
     $.ajax({
		...
		success:function (data) {
			...
			form.setData({
		                ...
		                url : mm.url,
		                image : mm.image,
		                whetherpublic : mm.whetherpublic,
		                actiontype : mm.actiontype,
		                remark : mm.remark
		            });
		       if(mm.pid != "0"){
		       		form.setData({
		...
		error:function (error) {
			top.$.ligerDialog.error("获取单个菜单信息失败****" + error.status);
	}});
  }   
    </script>
    <style type="text/css">
