...
function my_initGrid(){
   ...
   */
function  itemclick(){
	var url = "system/dictionary/DataDictionaryAdd.jsp";
	winOpen(url,'添加数据字典',450,280,'添加','取消',function(data){
       	$.ajax({
			url:"dictAdd.action", 
			data:data,
			dataType:"json", 
			...
			success:function (mm) {
       			if("error"==mm.result){
       				top.my_alert("添加数据字典信息失败!");
       			}else{
	       			g.addRow(mm);
	       			top.$.ligerDialog.success("添加数据字典信息成功!","success");
       			}
			}, 
			error:function (error) {
				top.my_alert("添加数据字典信息失败!" + error.status,"error");
		}});
	});
}
...
function  itemedit(){
	...
	var url = "system/dictionary/DataDictionaryEdit.jsp?id="+id;
	winOpen(url,'修改数据字典',450,280,'修改','取消',function(data){
       	$.ajax({
			url:"dictEdit.action", 
			data:data,
			dataType:"json", 
			...
			success:function (mm) {
	       		if("error"==mm.result){
	   				top.my_alert("修改数据字典信息失败!");
	   			}else{
	   				g.updateRow(selected,mm);
       				top.$.ligerDialog.success("修改数据字典信息成功!","提示");
	   			}
			}, 
			error:function (error) {
				top.my_alert("修改数据字典信息失败!" + error.status,"error");
		}});
	});
}
    ...
    window.top.$.ligerDialog.confirm("确定删除选择的数据", "提示", function (ok) {
	    if (ok) {
			...
			$.ajax({
				...
				success:function (mm) {
		       		if("error"==mm.result){
		   				top.my_alert("删除数据字典信息失败!");
		   			}else{
		   				top.my_alert("删除数据字典信息成功!");
		   			}
				}, 
				error:function (error) {
					top.my_alert("删除数据字典信息失败!" + error.status);
			}});
	     }
	 });
}
