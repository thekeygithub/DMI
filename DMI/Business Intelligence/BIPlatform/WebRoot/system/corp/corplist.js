...
function my_initGrid(){
	window['g']=$("#maingrid").ligerGrid({
		checkbox: true,
		rowHeight:22,
		url:"corpQueryList.action",
		columns: [
		{ display: '公司名称', name: 'corpname', width:"30%",isSort:true},
		{ display: '公司编码', name: 'epid', width:"30%",isSort:true},
   ...
   */
   function  itemclick(){
      var url = "system/corp/CorpAdd.jsp?pid=0";
      winOpen(url,"添加公司信息",500,300,'添加','取消',function(data){
	       	$.ajax({
				...
				success:function (mm) {
		       		if("error"==mm.result){
	       				top.my_alert("公司公司信息失败！");
	       			}else{
	       				g.addRow(mm);
						top.$.ligerDialog.success("添加公司信息成功！");
	       			}
				}, 
   ...
   */
   function  itemedit(){
		...
		var url = "system/corp/CorpEdit.jsp?id="+id;
		winOpen(url,"修改公司信息",500,300,'修改','取消',function(data){
	     	$.ajax({
				...
				success:function (mm) {
		     		if("error"==mm.result){
	       				top.my_alert("修改公司信息失败！");
	       			}else{
	       				g.updateRow(selected,mm);
						top.$.ligerDialog.success("修改公司信息成功！");
	       			}
				}, 
				error:function (error) {
					top.$.ligerDialog.error("修改公司信息失败！");
				}
			});
		});
   }
		...
		window.top.$.ligerDialog.confirm("确定删除选择的数据", "提示", function (ok) {
			if (ok) {
				...
				$.ajax({
					...
					success:function (mm) {
			     		if("error"==mm){
		       				top.my_alert("所选公司中有未选中的子公司或所属部门，请重新勾选！");
		       			}else{
					...
					error:function (error) {
						top.$.ligerDialog.error("删除公司信息失败！");
					}
				});
			}
		});
