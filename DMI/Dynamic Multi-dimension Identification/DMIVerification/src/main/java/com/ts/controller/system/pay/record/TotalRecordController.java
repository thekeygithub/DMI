...
public class TotalRecordController extends BaseController {
	...
	@Rights(code = "record/totalList")
	public ModelAndView totalList(Page page) throws Exception {
		...
		mv.setViewName("pay/record/businessTotal/show_total_list");
	...
	@Rights(code = "record/totalDetail")
	public ModelAndView totalDetail(Page page) throws Exception {
		...
		page.setPd(pd);
		List<PageData> varList = precordService.searchTotalDetail(page);
		mv.setViewName("pay/record/businessTotal/total_detail_page");
		mv.addObject("varList", varList);
		mv.addObject("pd", pd);
		return mv;
	}
	
	@RequestMapping(value = "/totalSituation1")
	@Rights(code = "record/totalSituation1")
	public ModelAndView totalSituation1() throws Exception {
		ModelAndView mv = this.getModelAndView();
		PageData pd = this.getPageData();
		pd.put("DATA_TYPE", "1");//对账详情
		PageData varModel = precordService.findTotalById(pd);
		mv.setViewName("pay/record/businessTotal/show_situation_list1");
	...
	@Rights(code = "record/exporExcel")
	public ModelAndView exporExcel() {
		logBefore(logger, Jurisdiction.getUsername()+"导出业务总额对账数据");
		ModelAndView mv = this.getModelAndView();
		PageData pd = this.getPageData();
		pd = this.getPD(pd);
		pd.put("DATA_TYPE", "1");//业务总额对账
