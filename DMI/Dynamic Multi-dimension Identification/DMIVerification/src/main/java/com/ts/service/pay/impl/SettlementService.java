...
public class SettlementService implements SettlementManager {
	...
	public List<PageData> getI31DataByDate(PageData pd) throws Exception {
		return (List<PageData>) dao.findForList("PtotaldetailMapper.i31DataByDate", pd);
	}
	...
	public List<PageData> getI49DataByDate(PageData pd) throws Exception {
		return (List<PageData>) dao.findForList("PtotaldetailMapper.i49DataByDate", pd);
	}
	/* (non-Javadoc)
	...
	public void settlement(Date startDate, Date endDate, String groupId, String hospCode) throws Exception{
		...
		List<PageData> i31List = getI31DataByDate(pd);
		List<PageData> i49List = getI49DataByDate(pd);
	...
	private P_total_detail getTotalDetailByPageDate(PageData pageData, Date dataDate, Map<String, String> sysUserMap) {
		...
		totalDetail.setCHECK_TYPE(1);//0门诊和住院，1门诊，2住院 
		//totalDetail.setCONFIRM_NUM(null);
		//totalDetail.setPAY_NUM(null);
		//totalDetail.setRETURN_NUM(null);
		totalDetail.setDATA_TYPE(2);
		totalDetail.setFINAL_DATE((Date) pageData.get("CREATEDATE"));//这里存数据的生成时间
		totalDetail.setFINAL_NO(pageData.getString("FINAL_NO"));
		totalDetail.setGROUP_ID(pageData.getString("GROUP_ID"));
		totalDetail.setGROUP_NAME(sysUserMap.get(pageData.getString("GROUP_ID")));
		totalDetail.setHOS_CODE(pageData.getString("HOS_CODE"));
		totalDetail.setHOS_NAME(pageData.getString("HOS_NAME"));
		totalDetail.setID(uuid);
		totalDetail.setMED_FEE(((BigDecimal)pageData.get("MED_FEE")).doubleValue());
		totalDetail.setREQ_NO(pageData.getString("REQ_NO"));
		totalDetail.setRETURN_FEE(((BigDecimal) pageData.get("RETURN_FEE")).doubleValue());//
		totalDetail.setSELF_NEG(((BigDecimal) pageData.get("SELF_NEG")).doubleValue());
		totalDetail.setSELF_PAY(((BigDecimal) pageData.get("SELF_PAY")).doubleValue());
		totalDetail.setTIME_STAMP(pageData.getString("TIME_STAMP"));//
		totalDetail.setTOTAL_FEE(((BigDecimal) pageData.get("TOTAL_FEE")).doubleValue());
		totalDetail.setVISIT_NO(pageData.getString("VISIT_NO"));
		totalDetail.setDATA_DATE(dataDate);
		totalDetail.setCASH_TOTAL(((BigDecimal) pageData.get("CASH_TOTAL")).doubleValue());
		return totalDetail;
	}
	...
	private P_return_detail getReturnDetailByPageDate(PageData pageData, Date dataDate, Map<String, String> sysUserMap) {
		...
		returnDetail.setCHECK_TYPE(1);//0门诊和住院，1门诊，2住院 
		//returnDetail.setCONFIRM_NUM(null);
		//returnDetail.setPAY_NUM(null);
		//returnDetail.setRETURN_NUM(null);
		returnDetail.setDATA_TYPE(2);
		returnDetail.setFINAL_DATE((Date) pageData.get("CREATEDATE"));//这里存数据的生成时间
		returnDetail.setFINAL_NO(pageData.getString("FINAL_NO"));
		...
		returnDetail.setHOS_CODE(pageData.getString("HOS_CODE"));
		returnDetail.setHOS_NAME(pageData.getString("HOS_NAME"));
		returnDetail.setID(uuid);
		returnDetail.setMED_FEE(((BigDecimal) pageData.get("MED_FEE")).doubleValue());
		returnDetail.setREQ_NO(pageData.getString("REQ_NO"));
		returnDetail.setRETURN_FEE(((BigDecimal) pageData.get("RETURN_FEE")).doubleValue());//
		returnDetail.setSELF_NEG(((BigDecimal) pageData.get("SELF_NEG")).doubleValue());
		returnDetail.setSELF_PAY(((BigDecimal) pageData.get("SELF_PAY")).doubleValue());
		returnDetail.setTIME_STAMP(pageData.getString("TIME_STAMP"));//
		returnDetail.setTOTAL_FEE(((BigDecimal) pageData.get("TOTAL_FEE")).doubleValue());
		returnDetail.setVISIT_NO(pageData.getString("VISIT_NO"));
		returnDetail.setDATA_DATE(dataDate);
		returnDetail.setCASH_TOTAL(((BigDecimal) pageData.get("CASH_TOTAL")).doubleValue());
		return returnDetail;
	}
