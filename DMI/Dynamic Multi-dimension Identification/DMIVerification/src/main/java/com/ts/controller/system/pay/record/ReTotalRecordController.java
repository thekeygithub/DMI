...
public class ReTotalRecordController extends BaseController {
	...
	@RequestMapping(value = "/reTotalList")
	@Rights(code = "record/reTotalList")
	public ModelAndView reTotalList(Page page) throws Exception {
		...
		pd = this.getPD(pd);
		pd.put("DATA_TYPE", "1");//退费对账
		page.setPd(pd);
		List<PageData> varList = precordService.queryReTotalList(page);
		
		//========================== start ======================================
		Double fee_sum = 0.0;//退费费用总额累加
		Double self_fee_sum = 0.0;//退费自费总额(非医保)累加
		Double drug_type_sum = 0.0;//退费药品乙类自负累加
		Double hosption_sum = 0.0;//退费医保费用累加
		Double reimburse_sum = 0.0;//退费合计报销金额累加
		List<PageData> recordList = precordService.queryReTotalListAll(pd);
		...
		mv.setViewName("pay/record/returnTotal/show_reTotal_list");
		mv.addObject("varList", varList);
		mv.addObject("pd", pd);
		return mv;
	}
	
	@RequestMapping(value = "/reTotalDetail")
	@Rights(code = "record/reTotalDetail")
	public ModelAndView reTotalDetail(Page page) throws Exception {
		ModelAndView mv = this.getModelAndView();
		PageData pd = this.getPageData();
		pd = this.getPD(pd);
		pd.put("DATA_TYPE", "2");//退费明细对账
		page.setPd(pd);
		List<PageData> varList = precordService.queryReDetail(page);
		mv.setViewName("pay/record/returnTotal/re_detail_page");
		mv.addObject("varList", varList);
		mv.addObject("pd", pd);
		return mv;
	}
	
	@RequestMapping(value = "/reTotalSituation3")
	@Rights(code = "record/reTotalSituation3")
	public ModelAndView reTotalSituation3() throws Exception {
		ModelAndView mv = this.getModelAndView();
		PageData pd = this.getPageData();
		pd.put("DATA_TYPE", "1");
		PageData varModel = precordService.findReturnById(pd);
		mv.setViewName("pay/record/returnTotal/show_situation_list3");
	...
	@RequestMapping(value = "/reExporExcel")
	@Rights(code = "record/reExporExcel")
	public ModelAndView reExporExcel() {
		logBefore(logger, Jurisdiction.getUsername()+"导出退费对账数据");
		ModelAndView mv = this.getModelAndView();
		PageData pd = this.getPageData();
		pd = this.getPD(pd);
		pd.put("DATA_TYPE", "1");//退费对账
		try {
			...
			Double hosption_sum = 0.0;//退费医保费用累加
			Double reimburse_sum = 0.0;//退费合计报销金额累加
			List<PageData> list = precordService.queryReTotalListAll(pd);
