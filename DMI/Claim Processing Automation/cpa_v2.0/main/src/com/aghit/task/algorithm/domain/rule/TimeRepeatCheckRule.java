...
// * 
// */
//public class TimeRepeatCheckRule extends AbstractRule {
//
...
//			
//			// 如果时间有重复，记录违规数据
//			int days = isTimeRepeatAdvice(recordDate, compareMap);
//			if(days != -1){
...
//	 * @return
//	 */
//	private List<BreakDetail> handlePrescription(Map<Date, Payment> handleData) {
//		
//		// 返回的违规信息
//		List<BreakDetail> retInfo = new ArrayList<BreakDetail>();
//		// 如果无处理数据直接返回
...
//			Entry<Date, Payment> ele = it.next();
//			Date recordDate = ele.getKey();		// 药品结算时间
//			Prescription pre = (Prescription) ele.getValue();
//			// 如果非处理数据，跳出当前循环进行下一条处理
//			if(!pre.getSign()){
//				continue;
//			}
//			
//			// 如果数据不完整，直接跳出当前循环进行下一条处理
//			if (pre.getDrug_amount() == 0 || pre.getDrug_usage() == 0
//					|| StringUtils.isBlank(pre.getDrug_usage_unit())
//					|| pre.getDrug_dosage() == 0) {
//				continue;
//			}
//			
//			checkStartDt = DateUtils.addDays(CPADateUtil.getDateFirst(recordDate), -cycleEnd);//设置周期开始日
...
//			
//			// 如果时间有重复，记录违规数据
//			int days = isTimeRepeatPre(recordDate, compareMap);
//			if(days != -1){
//				BreakDetail error = new BreakDetail();
//				error.setRuleId(this.ruleId);	// 规则Id
//				error.setRawDataId(pre.getSi_clearing_cost_no());	// 原始记录Id
//				error.setBreakDetial("开药时间重复,上次取药天数为:" + days); // 错误信息
//				retInfo.add(error);			//添加到违规总记录中
//			}
//		}
//		
...
//	 * 计算是否有重复诊疗、耗材
//	 * 
//	 * @param recordDate 新开诊疗日期
//	 * @param compareMap 比较周期之内的同种诊疗、耗材
//	 * @return 无违规数据返回-1，否则返回取治疗天数
//	 */
//	private int isTimeRepeatAdvice(Date recordDate, Map<Date,Payment> compareMap){
//		
//		Iterator<Entry<Date, Payment>> it = compareMap.entrySet().iterator();
//		// 循环处理相关数据
//		while(it.hasNext()){
//			Entry<Date, Payment> ele = it.next();
//			Date currDt = ele.getKey();		// 药品结算时间
//			DoctorAdvice advice = (DoctorAdvice) ele.getValue();
//			
//			// 计算治疗天数
//			int dayOfUsage = countDaysOfUsage(advice);
//			
//			Date noRepeatDt = DateUtils.addDays(CPADateUtil.getDateFirst(currDt), dayOfUsage);
//			// 如果新开诊疗日期，在前次诊疗的治疗天数之内，报错
//			if(noRepeatDt.compareTo(CPADateUtil.getDateFirst(recordDate)) >= 0){
//				return dayOfUsage;
//			}
//		}
//		
...
//	 * 计算是否有重复开药
//	 * 
//	 * @param recordDate 新开药日期
//	 * @param compareMap 比较周期之内的同种药物
//	 * @return 无违规数据返回-1，否则返回取药天数
//	 */
//	private int isTimeRepeatPre(Date recordDate, Map<Date,Payment> compareMap){
//		
//		Iterator<Entry<Date, Payment>> it = compareMap.entrySet().iterator();
//		// 循环处理相关数据
//		while(it.hasNext()){
...
//			
//			// 计算取药天数
//			int dayOfUsage = countDaysOfUsage(pre);
//			
//			Date noRepeatDt = DateUtils.addDays(CPADateUtil.getDateFirst(currDt), dayOfUsage);
//			// 如果新开药品日期，在药品的取药天数之内，报错
//			if(noRepeatDt.compareTo(CPADateUtil.getDateFirst(recordDate)) >= 0){
//				return dayOfUsage;
//			}
//		}
...
//	 * 计算诊疗、耗材的最少治疗天数
//	 * 
//	 * 计算公式：(数量/知识库频次最大用量)*频次天数
//	 * 
//	 * @param advice 诊疗信息
//	 * @return
//	 */
//	private int countDaysOfUsage(DoctorAdvice advice){
//			
...
//		case 1:
//			// 使用 -- 频次单位最大剂量计算
//			klDayDose = klItem.getMaxDoseFreqUnit() / klItem.getDose() ;
//			break;
//		case 2:
//			// 使用 -- 协定日剂量计算
//			klDayDose = klItem.getFixDayDose() / klItem.getDose();
//			break;
//		default:
//			// 使用 -- 医保日剂量计算
//			klDayDose = klItem.getSiFixDayDose() / klItem.getDose();
//		}
//		
//		return (int) Math.ceil((advice.getOrder_amount()/klDayDose)* getFreqUnitDays(klItem.getFreqUnit()));
//	}
//	
//	/**
//	 * 计算药品的实际用药天数
//	 * 
//	 * 计算公式：数量/(用法（每日）*用量)或者(知识库最大日剂量/剂量规格数)
//	 * 上述公式中，分母谁数值小选取谁参与运算，运算结果取整
//	 * 
//	 * @param pre 药品信息
//	 * @return
//	 */
//	private int countDaysOfUsage(Prescription pre){
//			
//		double klDayDose = 0;	// 知识库用药日剂量
//		double factDayDose = 0;	// 实际用药日剂量
//		
//		// 判断日剂量所选用字段(可选有3个)，并统一计算到日
//		// 知识库日剂量 = 频次单位最大剂量/(剂量规格数*频次天数)
//		switch (dayDoseCalcWay) {
//		case 1:
//			// 使用 -- 频次单位最大剂量计算
//			klDayDose = klItem.getMaxDoseFreqUnit()
//					/ (klItem.getDose() * getFreqUnitDays(klItem.getFreqUnit()));
//			break;
//		case 2:
//			// 使用 -- 协定日剂量计算
//			klDayDose = klItem.getFixDayDose()
//					/ (klItem.getDose() * getFreqUnitDays(klItem.getFreqUnit()));
//			break;
//		default:
//			// 使用 -- 医保日剂量计算
//			klDayDose = klItem.getSiFixDayDose()
//					/ (klItem.getDose() * getFreqUnitDays(klItem.getFreqUnit()));
//		}
//		
//		// 实际用药日剂量 = (用法*用量)/周期
//		factDayDose = (pre.getDrug_usage() * pre.getDrug_dosage())
//				/ getFreqUnitDays(pre.getDrug_usage_unit());
//		
//		// 参与运算的日剂量
//		double dayDose = factDayDose > klDayDose ? klDayDose : factDayDose;
//		
//		return (int) Math.ceil(pre.getDrug_amount()/dayDose);
//	}
//
//	/**
//	 * 找出与本规则相关的有用数据
