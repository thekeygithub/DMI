...
@Service("loadOupDataService")
public class LoadOupDataServiceImpl extends BaseService implements LoadMidDataService{
	...
	public Map<String, Collection<Map<String, Object>>> executeFetchMidData(
		...
		log.debug("全部门诊数据处理消耗时间："+ (System.currentTimeMillis()- st)); 
		return result;
	}
	...
	private Map<String,Map<String,Object>> loadClmDdata(Date sdt, Date edt, long sseq, long eseq, long src) throws Exception{
		...
		sql.append("       ,CASE WHEN fy.clm_id = p.clm_id OR p.clm_id IS NULL THEN 1 ELSE 0 END sign, fy.MED_DATE, fy.MED_DATE ");
		sql.append("       ,h.HOSP_ORG_TYPE_ID, fy.CLE_FORM_ID, fy.P_MI_CAT_ID ");
		sql.append("  FROM MID_OUP_CLM fy ");
	    ...
	    if (false == Constant.EXE_TYPE_REALTIME.equals(this.exeType)){ //即时执行不判断mts_flag
		    ...
		    sql.append(" AND NOT EXISTS (SELECT 1 FROM mid_oup_pre WHERE mts_flag <> '1' AND clm_id = fy.clm_id AND data_src_id = fy.data_src_id) ");
	        sql.append(" AND NOT EXISTS (SELECT 1 FROM mid_oup_ord WHERE mts_flag <> '1' AND clm_id = fy.clm_id AND data_src_id = fy.data_src_id) ");
	    }
	    ...
	    log.debug("加载门诊Mid结算单消耗时间："+ (System.currentTimeMillis()- st) + "，记录数：" + res.size());   
	...
	private List<Map<String,Object>> loadMtsClmDdata() throws Exception{
		...
		sql.append("  FROM MTS_OUP_CLM_DIAG d, CPA_TP_CLMID tp ");
	    ...
	    log.debug("加载门诊MTS结算单消耗时间："+ (System.currentTimeMillis()- st) + "，记录数：" + res.size());   
	...
	private Collection<Map<String,Object>> assemblePreData(Map<String,Map<String,Object>> clmDt) throws Exception{
		...
		log.debug("全部门诊处方处理消耗时间："+ (System.currentTimeMillis()- st) + "，记录数：" + detail.size());
	...
	private Collection<Map<String,Object>> assembleOrdData(Map<String,Map<String,Object>> clmDt) throws Exception{
		...
		log.debug("全部门诊医嘱处理消耗时间："+ (System.currentTimeMillis()- st) + "，记录数：" + detail.size());
		return detail.values();
	}
	...
	private Collection<Map<String,Object>> assembleClmData(Map<String,Map<String,Object>> clmMidDt) throws Exception{
		...
		log.debug("全部门诊诊断处理消耗时间："+ (System.currentTimeMillis()- st) + "，记录数：" + mts.size());
	...
	private Map<String,Map<String,Object>> loadMidOrdData() throws Exception{
		...
		sql.append("  FROM MID_OUP_ORD ord, CPA_TP_CLMID tp ");
	    ...
	    log.debug("加载门诊MID医嘱消耗时间："+ (System.currentTimeMillis()- st) + "，记录数：" + res.size());   
	...
	private Map<String,Map<String,Object>> loadMtsOrdData() throws Exception{
		...
		sql.append("  FROM MTS_OUP_ORD ord, CPA_TP_CLMID tp ");
		sql.append(" WHERE tp.CLM_ID = ord.CLM_ID");
	    sql.append("   AND ord.DATA_SRC_ID = tp.DATA_SRC_ID");
	    ...
	    log.debug("加载门诊MTS医嘱消耗时间："+ (System.currentTimeMillis()- st) + "，记录数：" + res.size());   
	...
	private Map<String,Map<String,Map<String,Object>>> loadTotalOrdData() throws Exception{
		...
		sql.append(" INNER JOIN MTS_OUP_ORD MTS ON OBJ.OBJ_ID = MTS.AG_ID");		
		sql.append(" INNER JOIN MID_OUP_ORD MID ON MTS.ORD_ID = MID.ORD_ID AND MTS.DATA_SRC_ID = MID.DATA_SRC_ID");	
	...
	private Map<String,Map<String,Object>> loadMidPreData() throws Exception{
		...
		sql.append("  FROM MID_OUP_PRE pre, CPA_TP_CLMID tp ");
	    ...
	    log.debug("加载门诊MID处方消耗时间："+ (System.currentTimeMillis()- st) + "，记录数：" + res.size());   
	...
	private Map<String,Map<String,Object>> loadMtsPreData() throws Exception{
		...
		sql.append("  FROM MTS_OUP_PRE pre, CPA_TP_CLMID tp ");
		sql.append(" WHERE tp.CLM_ID = pre.CLM_ID");
	    sql.append("   AND pre.DATA_SRC_ID = tp.DATA_SRC_ID");
	    ...
	    log.debug("加载门诊MTS处方消耗时间："+ (System.currentTimeMillis()- st) + "，记录数：" + res.size());   
	...
	private Map<String,Map<String,Map<String,Object>>> loadTotalPreData() throws Exception{
		...
		sql.append(" FROM K_DEF DEF");		
		sql.append(" INNER JOIN K_OBJ OBJ ON DEF.K_ID = OBJ.K_ID");		
		sql.append(" INNER JOIN MTS_OUP_PRE MTS ON OBJ.OBJ_ID = MTS.DRUG_ID");		
		sql.append(" INNER JOIN MID_OUP_PRE MID ON MTS.PRE_ID = MID.PRE_ID AND MTS.DATA_SRC_ID = MID.DATA_SRC_ID");	
		sql.append(" INNER JOIN K_ELEM_DEF ELEM ON DEF.K_ID = ELEM.K_ID AND ELEM_TYPE_ID IN ('301','302','311')");		
		sql.append(" INNER JOIN CPA_TP_CLMID TP ON MTS.CLM_ID = TP.CLM_ID AND MTS.DATA_SRC_ID = TP.DATA_SRC_ID")	;	
